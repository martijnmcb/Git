...
# Simuleer scenario
def generate_scenario_df(aantal_taxi=10, short=200, medium=50, long=50, scenario="Short First"):
    planning = []
    ...
    if scenario == "Short First":
        ritten.sort(key=lambda x: x[2])
    elif scenario == "Long First":
        ritten.sort(key=lambda x: -x[2])
    # anders: originele volgorde (Medium First of custom)
...
# Scenario keuze
scenario = st.selectbox("Scenario volgorde ritten", ["Short First", "Long First", "Original"])
...
df = generate_scenario_df(aantal_taxi, short, medium, long, scenario)
