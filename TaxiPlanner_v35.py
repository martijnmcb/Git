...
# Simuleer scenario met mix én voorkeurstype per taxi
def generate_scenario_df(aantallen, duren, groepen, mix_mode=False):
    planning = []
    kleuren = {"Short": "green", "Medium": "orange", "Long": "red"}

    if mix_mode:
        # Maak een taxi-pool met voorkeuren
        taxi_id = 1
        taxi_slots = {}  # taxi_id: tijd
        taxi_pref = {}   # taxi_id: "Short"/"Medium"/"Long"
        for rit_type in ["Short", "Medium", "Long"]:
            for _ in range(groepen[rit_type]):
                taxi_slots[taxi_id] = 0
                taxi_pref[taxi_id] = rit_type
                taxi_id += 1

        # Maak een rittenlijst per type
        ritten_dict = {}
        for typ in ["Short", "Medium", "Long"]:
            ritten_dict[typ] = [{"type": typ, "duur": duren[typ]} for _ in range(aantallen[typ])]

        total_ritten = sum(len(ritten_dict[typ]) for typ in ritten_dict)

        while total_ritten > 0:
            taxi = min(taxi_slots, key=taxi_slots.get)
            voorkeur = taxi_pref[taxi]
            rit = None

            # Probeer eerst eigen voorkeur
            if ritten_dict[voorkeur]:
                rit = ritten_dict[voorkeur].pop(0)
            else:
                # Pak alternatief type
                for typ in ["Short", "Medium", "Long"]:
                    if ritten_dict[typ]:
                        rit = ritten_dict[typ].pop(0)
                        break
            ...
            if rit:
                ...
                planning.append({
                    ...
                    "Duur": rit["duur"],
                    "WaitTime": start,
                    "Kleur": kleuren.get(rit["type"], "gray")
                })
                taxi_slots[taxi] = end
                total_ritten -= 1
...
# Streamlit UI
st.title("🚕 Taxi Ritplanner – versie 35")

mix_mode = st.checkbox("↔️ Mix ritcategorieën tussen taxi’s (eerst eigen type, dan flexibel)", value=True)
...
# Taxi verdeling
if mix_mode:
    st.markdown("### Aantal taxi’s per voorkeurstype")
    taxi_short = st.slider("Taxi's met voorkeur voor korte ritten", 0, 30, 4)
    taxi_medium = st.slider("Taxi's met voorkeur voor middellange ritten", 0, 30, 3)
    taxi_long = st.slider("Taxi's met voorkeur voor lange ritten", 0, 30, 3)
    groepen = {"Short": taxi_short, "Medium": taxi_medium, "Long": taxi_long}
else:
    st.markdown("### Verdeling taxi's over ritgroepen (niet mixen)")
