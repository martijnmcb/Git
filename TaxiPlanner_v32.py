...
# Simuleer scenario
def generate_scenario_df(aantal_taxi=10, aantallen=None, duren=None, volgorde=None):
    planning = []
    taxi_slots = {taxi: 0 for taxi in range(1, aantal_taxi + 1)}

    ritten = [(typ, aantallen[typ], duren[typ]) for typ in volgorde]
...
volgorde_opties = {
    ...
    "Medium → Short → Long": ["Medium", "Short", "Long"]
}

scenario_keuze = st.selectbox("Scenario volgorde ritten", list(volgorde_opties.keys()))
volgorde = volgorde_opties[scenario_keuze]
...
df = generate_scenario_df(aantal_taxi, aantallen, duren, volgorde)
